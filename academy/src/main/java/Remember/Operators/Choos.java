...
public class Choos {
	...
	public static class Main {
		...
		public static void findShortestAndLongestString(String[] sourceStrings) {
			...
			println("Самая короткая строка (длина): " + shortest + "(" + shortest.length() + ")");
			println("Самая длинная строка (длина): " + longest + "(" + longest.length() + ")");
		}

		public static Map<String, Integer> sortStringsWithAscOrDescByLength(Map<String, Integer> map,
				char directionSign) {
			List<Entry<String, Integer>> forSortBylength = new ArrayList<Entry<String, Integer>>(map.entrySet());
			switch (directionSign) {
			case '>':
				Collections.sort(forSortBylength, new Comparator<Entry<String, Integer>>() {
				...
				Collections.sort(forSortBylength, new Comparator<Entry<String, Integer>>() {
					public int compare(Entry<String, Integer> o1, Entry<String, Integer> o2) {
						return ((Entry<String, Integer>) (o1)).getValue()
								.compareTo(((Entry<String, Integer>) (o2)).getValue());
					}
				});
				break;
			default:
		...
		public static void findStringsLengthByAscAndDesc(String[] sourceStrings) {
			...
			println("Упорядоченные строки в порядке возрастания значений " + "их длины (строка=длина): "
					+ sortStringsWithAscOrDescByLength(map, '<'));
			println("Упорядоченные строки в порядке убывания значений " + "их длины (строка=длина): "
					+ sortStringsWithAscOrDescByLength(map, '>'));
		}
		...
		public static void findStringsLessAndGreaterThanAverageLength(String[] sourceStrings) {
			...
			println("Строки, длина которых < средней(" + averageStringsLength + ") (строка=длина): "
					+ stringsLessThanAverageLength);
			println("Строки, длина которых > средней(" + averageStringsLength + ") (строка=длина): "
					+ stringsGreaterThanAverageLength);
		}
		...
		public static void findWordsWithEqualNumberOfVowelsAndConsonantsInLatin(String[] sourceWords) {
			List<String> wordsInLatin = findWordsInLatin(sourceWords);
			println("Слова, содержащие только символы латинского алфавита (слов: " + wordsInLatin.size() + "): "
					+ wordsInLatin);
		...
		public static void findWordWithOnlyDifferentSymbols(String[] sourceWords) {
			for (String word : sourceWords) {
				if (word.length() > 1 && hasUniqueChars(word)) {
					println("Слово, состоящее только из различных символов: " + word);
					break;
				}
			}
		}
		...
		public static void findFirstOrSecondNumericPalindromicWord(String[] sourceWords) {
			...
			for (String word : sourceWords) {
				if (isNumericWord(word)) {
					boolean isInputPalindrome = word.equals(new StringBuilder(word).reverse().toString()) ? true
							: false;
		 ...
		 * @since 1.7
		 */
//		public static void isBelongToInterval(int[] kSourceValues, String... intervals) {
...
//		}
		...
		public static void fillQuadMatrixFromLeftToRightOrDownwardForInt(int[][] matrix, char fillDirection) {
...
//			isBelongToInterval(kSourceValues, "(-10000, 5]", "[0, 10]", "[5, 15]", "[10, 10000]");
